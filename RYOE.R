...
pbp_all <- pbp_rp %>%
  inner_join(rushes_all2,by=c("yard_line","period","down","distance","score_diff","ep_before","wp_before","game_seconds_remaining")) %>%
  select(year,pos_team,def_pos_team,rusher_player_name,yards_gained,EPA,period,down,distance,yard_line,x_rush_yards) %>%
...
## inner join RYOE for all rushing plays and create updated RYOE file for each athlete ##

rusher_ryoe <- pbp_all2 %>%
  filter(!is.na(rusher_player_name)) %>%
  group_by(id_play,game_id) %>%
  slice_max(order_by = ryoe,n=1) %>%
  group_by(rusher_player_name,pos_team) %>%
  summarize(n=n(),yds = mean(yards_gained), total_yds = sum(yards_gained),tds = sum(touchdown), x_rush = mean(x_rush_yards), x_total = sum(x_rush_yards), ryoe = mean(ryoe)) %>%
  arrange(-ryoe)
